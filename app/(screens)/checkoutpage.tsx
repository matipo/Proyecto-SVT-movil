...
export default function CheckoutPage() {
  ...
  const purchase = params.purchase ? JSON.parse(params.purchase as string) : null;
      ...
      const saveHistory = async () => {
        try {
          ...
          const record = {
            ...
            eventId: purchase.event_name || purchase.event_id,
  ...
  if (!purchase) {
    return (
      <View className="flex-1 items-center justify-center bg-white">
        <Text className="text-lg font-semibold text-red-500">No se encontró el comprobante.</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView className="flex-1 bg-white px-5 pt-6">
      ...
      <View className="mb-5">
        ...
        <Text className="text-lg text-black">{purchase.event_name || purchase.event_id}</Text>
      </View>
      ...
      <View className="mb-6">
        ...
        {purchase.tickets?.map((t: any, idx: any) => (
          <View key={idx} className="mb-3 rounded-xl border border-gray-200 bg-gray-100 p-4">
      ...
      <View className="mb-6">
        <Text className="font-semibold text-gray-500">Fecha de Confirmación:</Text>
        <Text className="text-lg">{new Date(purchase.confirmed_at).toLocaleString()}</Text>
      </View>

      <Link href="/" className="mb-12 w-full rounded-xl bg-red-600 py-4">
        <Text className="text-center text-lg font-semibold text-white">Volver al Inicio</Text>
      </Link>
    </ScrollView>
  );
}